    ...
    <section class="mx-auto max-w-7xl px-4 py-10 md:px-6" x-data="{ policyModal: null }">
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <div class="lg:col-span-2 rounded-3xl border border-emerald-200 bg-[#f2fbf7] p-6 md:p-8">
                ...
                <p class="mt-2 text-sm text-slate-500">En kısa sürede görüşmek üzere 👋</p>
                ...
                <form method="POST" action="{{ route('volunteer.submit') }}" class="mt-6 space-y-4">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <input name="first_name" type="text" value="{{ old('first_name') }}" placeholder="Ad Soyad" required class="w-full rounded-full border border-slate-300 bg-white px-4 py-2.5 text-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                        <input name="email" type="email" value="{{ old('email') }}" placeholder="E-posta" required class="w-full rounded-full border border-slate-300 bg-white px-4 py-2.5 text-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                        <input name="last_name" type="text" value="{{ old('last_name') }}" placeholder="Soyad" required class="w-full rounded-full border border-slate-300 bg-white px-4 py-2.5 text-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                        <input name="phone" type="text" value="{{ old('phone') }}" placeholder="Telefon" required class="w-full rounded-full border border-slate-300 bg-white px-4 py-2.5 text-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                    </div>
                    ...
                    <select name="preference" required class="w-full rounded-full border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-700 outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                        <option value="">Gönüllülük Alan Tercihi</option>
                        @foreach(($volunteerPreferenceOptions ?? []) as $option)
                            <option value="{{ $option }}" @selected(old('preference') === $option)>{{ $option }}</option>
                        @endforeach
                    </select>

                    <textarea name="about" rows="7" placeholder="Kendinizi kısaca tanıtır mısınız, nasıl yardımcı olabilirsiniz?" required class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">{{ old('about') }}</textarea>
                    ...
                    <label class="flex items-start gap-2 text-sm text-slate-700">
                        ...
                        <span>
                            <button type="button" @click="policyModal = 'volunteer'" class="font-semibold text-emerald-700 underline underline-offset-2 hover:text-emerald-800">
                                Gönüllü Başvuru Aydınlatma Metni
                            </button>
                            ’ni okudum, farkındayım.
                        </span>
                    </label>
            ...
            <aside class="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="space-y-6 text-slate-700">
                    <div class="border-b border-slate-100 pb-5 text-center">
                        ...
                        <p class="text-xl font-bold text-slate-900">Telefon</p>
                    ...
                    <div class="border-b border-slate-100 pb-5 text-center">
                        ...
                        <p class="text-xl font-bold text-slate-900">E-posta</p>
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-xl font-bold text-slate-900">Adres</p>
        ...
        <div x-show="policyModal" x-transition class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4" style="display: none;">
            <div @click.outside="policyModal = null" class="max-h-[85vh] w-full max-w-3xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl">
                <div class="mb-4 flex items-start justify-between gap-4">
                    <h3 class="text-xl font-bold text-slate-900" x-text="policyModal === 'kvkk' ? 'KVKK Aydınlatma Metni' : 'Gönüllü Başvuru Aydınlatma Metni'"></h3>
